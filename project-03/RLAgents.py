...
class PolicyAgent():
    ...
    def update(self, rewards, gradients):
        # Compute the discount cumulative future reward at each step
        G = [0] * len(rewards)
        for t in range(len(rewards)):
            Gt = 0
            steps = 0
            for r in rewards[t:]:
                Gt += self.gamma**steps * r
                steps += 1
            G[t] = Gt

        # Update the weights based upon the gradient and Gt
        # Theta_t <-- Theta_t + alpha * gamma^t * Gt * [] grad_theta(pi(at,  st, tehta)) / pi(at, st, theta) ]
        for t in range(len(gradients)):
            self.theta += self.lr * G[t] * gradients[t]

class AC2Agent():
    def __init__(self, env, lr_actor = 0.001, lr_critic = 0.001, gamma=0.99):
        self.env = env
        self.observation_space = env.observation_space
        self.action_space = env.action_space
        ...
        self.gamma = gamma
        self.actor = self.make_actor_model()
        self.critic = self.make_critic_model()

    def make_actor_model(self):
        model = keras.models.Sequential()
        model.add(keras.layers.Dense(24, input_dim=self.observation_space.shape[0], activation='relu'))
        model.add(keras.layers.Dense(self.action_space.n, activation='softmax'))
        model.compile(loss='mse', optimizer=keras.optimizers.Adam(lr=self.lr_actor))
        return model
    ...
    def make_critic_model(self):
        ...
        model.compile(loss="mse", optimizer=keras.optimizers.Adam(lr=self.lr_critic))
        return model
    ...
    def step(self, state):
        probs = self.policy(state)
        action = np.random.choice(self.action_space.n, p=probs.squeeze())
        return action

    def update(self, state, action, reward, next_state, done):
        target = np.zeros((1, 1))
        advantages = np.zeros((1, self.action_space.n))
